...
RSpec.describe ProductUpdatesMailer, type: :mailer do
  ...
  describe 'january_2023' do
    let(:user) { create(:user) }
    let(:mail) { described_class.january_2023(user) }

    before { create(:site_with_team, owner: user) }

    it 'renders the headers' do
      expect(mail.subject).to eq 'Product Update: January 2023'
      expect(mail.to).to eq [user.email]
      expect(mail.from).to eq ['hello@squeaky.ai']
    end
  ...
  describe 'february_2023' do
    let(:user) { create(:user) }
    let(:mail) { described_class.february_2023(user) }

    before { create(:site_with_team, owner: user) }

    it 'renders the headers' do
      expect(mail.subject).to eq 'Product Update: February 2023'
      expect(mail.to).to eq [user.email]
      expect(mail.from).to eq ['hello@squeaky.ai']
    end
  ...
  describe 'march_2023' do
    let(:user) { create(:user) }
    let(:mail) { described_class.march_2023(user) }

    before { create(:site_with_team, owner: user) }

    it 'renders the headers' do
      expect(mail.subject).to eq 'Product Update: March 2023'
      expect(mail.to).to eq [user.email]
      expect(mail.from).to eq ['hello@squeaky.ai']
    end
  ...
  describe 'changelog_public_roadmap' do
    let(:user) { create(:user) }
    let(:mail) { described_class.changelog_public_roadmap(user) }

    before { create(:site_with_team, owner: user) }

    it 'renders the headers' do
      expect(mail.subject).to eq 'Introducing Squeaky\'s New Changelog and Public Roadmap'
      expect(mail.to).to eq [user.email]
      expect(mail.from).to eq ['hello@squeaky.ai']
    end
