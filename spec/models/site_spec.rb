...
RSpec.describe Site, type: :model do
  ...
  describe '#create_authorizer!' do
    context 'when the site authorizer does not exist' do
      let(:user) { create_user }
      let(:site) { create_site_and_team(user: user) }
      let(:subject) { site.create_authorizer! }
    ...
    context 'when the site authorizer does exist' do
      let(:user) { create_user }
      let(:site) { create_site_and_team(user: user) }
      let(:subject) { site.create_authorizer! }
  ...
  describe '#delete_authorizer!' do
    context 'when the site authorizer does not exist' do
      let(:user) { create_user }
      let(:site) { create_site_and_team(user: user) }
      let(:subject) { site.delete_authorizer! }
